...
patches = read.csv("data/patch_data.csv")
...
x11()
pdf("simpopdyn.pdf", height=8, width=8, family="Times")
...
predP = matrix(NA, nrow=length(allCinxiadat)-1, ncol=nsim)
for(i in 1:(length(allCinxiadat)-1)){
  sums = apply(allCinxiadat[[i]][,4:nc], 2, sum, na.rm=T)
  ns = apply(allCinxiadat[[i]][,4:nc]>-1, 2, sum, na.rm=T)
  predP[i,] = sums/ns
}
...
plot(years, yearlyDat$MelC, type="b", pch=16, ylim=c(0,.6), xlab="", las=1, ylab="",
     main=expression("Butterfly"))
xx = 2001:2018
...
plot(years, yearlyDat$PhoPcomb, type="b", pch=16, ylim=c(0,.5), xlab="", las=1, ylab="",
     main=expression("Mildew"))
xx = 2001:2018
polygon(c(xx,rev(xx)), c(df$predPlower[1:18],rev(df$predPupper[1:18])), col=cols[2], border=FALSE)
points(df$Year, df$predP, pch=16, type="l", lwd=2, col="darkgrey")
points(years, yearlyDat$PhoPcomb, type="b", lwd=2, pch=16)

# Cotesia
predP = matrix(NA, nrow=length(allCotesiadat)-1, ncol=nsim)
...
plot(years, yearlyDat$CotPAdj, type="b", pch=16, ylim=c(0,.03), xlab="", ylab="", las=1,
     main=expression("Parasitoid"))
xx = 2001:2018
polygon(c(xx,rev(xx)), c(df$predPlower[1:18],rev(df$predPupper[1:18])), col=cols[3], border=FALSE)
points(df$Year, df$predP, pch=16, type="l", lwd=2, col="darkgrey")
points(years, yearlyDat$CotPAdj, type="b", lwd=2, pch=16)
...
for(i in 1:6){
  ...
  lines(rownames(wide), wide[,i], type="b", lwd=2, pch=16)
...
##### END OF ANALYSES INCLUDED IN MANUSCRIPT #####
